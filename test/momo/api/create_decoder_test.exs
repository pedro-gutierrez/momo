defmodule Momo.Api.CreateDecoderTest do
  use Momo.DataCase
  ...
  describe "create api decoder" do
    test "decodes attributes and relations", context do
      params = context.post_api_params

      assert {:ok, data} = Post.ApiCreateDecoder.decode(params)
    ...
    test "detects ids that are not uuids", context do
      params =
        context.post_api_params
        |> Map.put("id", "foo")
        |> Map.put("blog", %{"id" => "bar"})

      assert {:error, errors} = Post.ApiCreateDecoder.decode(params)
    ...
    test "detects unknown relations", context do
      params = Map.put(context.post_api_params, "blog", %{"id" => Ecto.UUID.generate()})
